...
public class ThirdPersonController2 : MonoBehaviour
{
	...
	// Handle rotation here to ensure smooth application.
	{
		...
		if (Input.GetMouseButton (0) && (!requireLock || controlLock || Screen.lockCursor))
	...
	// If the right mouse button is held, the horizontal axis also turns into sidestep handling
	{
		...
		{
			float st = 0;
			if (Input.GetKeyDown(KeyCode.Q)) st = -1;
			else if (Input.GetKeyDown(KeyCode.E)) st = 1;

			if (Input.GetMouseButton (1))
			{
				float sidestep = st, horizontal = Input.GetAxis ("Horizontal");
